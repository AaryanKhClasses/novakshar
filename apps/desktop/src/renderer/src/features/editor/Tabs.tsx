...
export function Tabs() {
    ...
    return <div className="flex items-center border-b border-border bg-editor text-sm text-text-alt overflow-x-auto">
        {documents.map(d => <div key={d.id} onClick={() => activateDocument(d.id)} className={`cursor-pointer h-full flex items-center justify-center gap-2 px-4 animate ${d.id === activeDocumentID ? 'bg-explorer hover:bg-explorer-selected text-text border-b-2 border-border-focus' : 'hover:bg-explorer-hover hover:text-text'}`}>
            {d.title}
