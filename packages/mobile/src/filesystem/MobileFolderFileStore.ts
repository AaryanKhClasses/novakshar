...
export class MobileFolderFileStore implements IFolderFileStore {
    constructor(
    ...
    public async create(folder: Folder): Promise<void> {
        ...
        await this.fileSystem.createDirectory(`${this.workspaceRoot}/${Constants.NotesFolder}/${relative}`)
    ...
    public async update(prev: Folder, current: Folder): Promise<void> {
        ...
        await this.fileSystem.move(
            `${this.workspaceRoot}/${Constants.NotesFolder}/${oldRelative}`,
            `${this.workspaceRoot}/${Constants.NotesFolder}/${newRelative}`
        )
        ...
        for(const document of documents) {
            ...
            if(!folder) continue
            const updatedRelativePath = `${this.resolver.resolve(folder)}/${this.basename(document.relativePath)}`
    ...
    public async delete(folder: Folder): Promise<void> {
        const relative = this.resolver.resolve(folder)
        await this.fileSystem.deleteDirectory(`${this.workspaceRoot}/${Constants.NotesFolder}/${relative}`)
